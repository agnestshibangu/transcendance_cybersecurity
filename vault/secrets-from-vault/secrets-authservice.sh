...
VAULT=${VAULT:-vault}
SECRETS_DIR=${SECRETS_DIR:-/secrets/authservice}
...
echo "Configuring AppRole for Authservice..."

# Écrire la politique pour Authservice
$VAULT policy write authservice /vault/authservice/authservice-policy.hcl
...
$VAULT write auth/approle/role/authservice \
    token_policies="authservice" \
    token_ttl=30m \
    token_max_ttl=60m

# Récupérer role_id
$VAULT read -format=json auth/approle/role/authservice/role-id \
    | jq -r '.data.role_id' > "$SECRETS_DIR/role_id"

# Récupérer secret_id
$VAULT write -format=json -f auth/approle/role/authservice/secret-id \
...
write_env_file "authservice" "secret/authservice"
