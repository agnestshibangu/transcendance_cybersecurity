...
VAULT=${VAULT:-vault}
SECRETS_DIR=${SECRETS_DIR:-/secrets/apigateway}
...
echo "Configuring AppRole for API Gateway..."

# Écrire la politique pour API Gateway
$VAULT policy write api-gateway /vault/api-gateway/api-gateway-policy.hcl
...
# Récupérer role_id
$VAULT read -format=json auth/approle/role/api-gateway/role-id \
    | jq -r '.data.role_id' > "$SECRETS_DIR/role_id"

# Récupérer secret_id
$VAULT write -format=json -f auth/approle/role/api-gateway/secret-id \
...
write_env_file "api-gateway" "secret/api-gateway"
